  ...
  public static void main(String[] args) {
    ...
    while (scanner.hasNextLine()) {
      ...
      // Request a ride
        ...
        String accid = "";
        System.out.print("User Acocount Id: ");
        if (scanner.hasNextLine()) {
        ...
        if (!tmuber.requestRide(accid, fradd, toadd))
          System.out.println(tmuber.getErrorMessage());
        else {
          String name = tmuber.getUser(accid).getName();
          System.out.printf("RIDE for: %-15s From: %-15s To: %-15s", name, fradd, toadd);
        }
      }
      ...
      // Request a food delivery
        ...
        if (scanner.hasNextLine()) {
          foodor = scanner.nextLine();
        }
        if (!tmuber.requestDelivery(accid, fradd, toadd, rest, foodor))
          System.out.println(tmuber.getErrorMessage());
        else {
          String name = tmuber.getUser(accid).getName();

          System.out.printf("DELIVERY for: %-15s From: %-15s To: %-15s", name, fradd, toadd);
        }
      }
